...
class EditorManager {
  ...
  async loadAPI(): Promise<void> {
    ...
    this.apiLoadingPromise = new Promise((resolve, reject) => {
      // 在加载任何 SDK 脚本之前，先设置 __ONLYOFFICE_SDK_BASE_PATH__
      // 确保 sdk-all-min.js 及其动态加载的子脚本能正确读取到这个值
      if (typeof window !== 'undefined') {
        const basePath = getOnlyOfficeSdkBasePath();
        // 始终使用 getOnlyOfficeSdkBasePath() 的值，确保配置一致
        window.__ONLYOFFICE_SDK_BASE_PATH__ = basePath;
        console.log('[OnlyOffice] loadAPI 中设置 __ONLYOFFICE_SDK_BASE_PATH__:', basePath);
        
        // 在加载 api.js 之前，设置 extensionParams 来指定正确的基础路径
        // 这样 OnlyOffice 就会使用我们指定的路径，而不是通过正则表达式匹配 script.src
        (window as any).extensionParams = {
          url: `${basePath}/packages/onlyoffice/7/web-apps/`,
        };
        console.log('[OnlyOffice] extensionParams 设置为:', (window as any).extensionParams.url);
      }
